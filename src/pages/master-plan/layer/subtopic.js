  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${API_ROUTES.sup_topic.get}/${id}`, {
          headers: {
      ...
      >
        <Container maxWidth="xl">
          <Stack spacing={3}>
            ...
            <PlanFile node={"sup-topic"} items={apiData} handleRemove={handleRemove} />
          </Stack>
        </Container>
      </Box>
...
SubTopic.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;
