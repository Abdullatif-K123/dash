  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${API_ROUTES.topic.get}/${id}`, {
          headers: {
      ...
      >
        <Container maxWidth="xl">
          <Stack spacing={3}>
            ...
            <PlanFile node={"topic"} items={apiData} handleRemove={handleRemove} />
          </Stack>
        </Container>
      </Box>
...
Topic.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;
